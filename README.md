...
| 💡 **提示** | 增强建议 | 比当前难度多检索一层，选中推荐棋子并标出走法；倒计时结束后自动取消 |
| 🔊 **音效** | 声音开关 | 开启/关闭游戏音效 |
...
检索耗时取决于当前局面的可选走法数量及设备性能。各档设置了思考时间上限：极速和标准适合手机端快速对局，困难通常在约两秒内返回，大师最多思考约五秒。提示会在当前档位上增加一层搜索上限并使用更长的独立预算。复杂局面不强求跑满搜索上限，中残局分支减少后会自动完成更深层次。
